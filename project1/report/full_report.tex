...
This report presents an exploratory data analysis (EDA) and simple linear regression study across three benchmark datasets from the UCI Machine Learning Repository: Auto MPG (392 rows, 8 columns), Concrete Compressive Strength (1{,}030 rows, 9 columns), and Airfoil Self-Noise (1{,}503 rows, 6 columns). For each dataset, preprocessing decisions (string columns, missing values, outliers), descriptive statistics, a correlation matrix and heatmap, and univariate \texttt{SimpleRegression} models for the top two target-correlated predictors are presented and discussed. Every regression was computed independently with both ScalaTion's \texttt{SimpleRegression} (Scala 3) and Python's \texttt{statsmodels} \texttt{OLS}; as shown below, the two tools agree to at least four decimal places on every coefficient, standard error, $R^2$, and $F$-statistic.
...
\begin{itemize}
    ...
    \item \textbf{Missing Values:} The raw 398-row file contains 6 rows with \texttt{?} in the \texttt{horsepower} field (pre-1980 records lacking a reported value). These 6 incomplete rows were dropped (not imputed), yielding the standard 392-row, 8-column benchmark matrix.
    \item \textbf{Outliers:} Tukey's 1.5$\times$IQR rule flags 10 outliers in \texttt{horsepower} (bounds $[-1.5, 202.5]$, all above 202.5~hp) and 11 in \texttt{acceleration} (bounds $[8.9, 21.9]$). These were retained rather than removed: they represent genuine high-performance vehicles, not data-entry errors, and removing them would bias the regression by truncating real automotive variation.
\end{itemize}
...
acceleration & 15.5413 & 2.7589 & 8.0000 & 13.7750 & 15.5000 & 17.0250 & 24.8000 & 0 & 11 \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{auto_mpg_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{auto_mpg_weight_regression.png}
...
\subsection{Preprocessing: String Values, Missing Values, and Outliers}
\begin{itemize}
    \item \textbf{String Values:} All 9 columns are numeric ingredient quantities (kg/m$^3$) or a curing duration (days); the only text encountered was the original Excel header row, which was renamed to clean snake\_case identifiers. No categorical/string encoding was required.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{concrete_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{concrete_cement_regression.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{concrete_superplasticizer_regression.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{airfoil_heatmap.png}
...
\textbf{Summary statement:} $\widehat{\text{scaled\_sound\_pressure\_db}} = 127.3037 - 0.0009 \cdot \text{frequency\_hz}$, with $R^2 = 0.1527$ (adj.\ $R^2 = 0.1521$), RMSE $= 6.3482$, on $n = 1503$ observations. Each 1-unit increase in \texttt{frequency\_hz} is associated with a $-0.0009$ change in \texttt{scaled\_sound\_pressure\_db}.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{airfoil_frequency_hz_regression.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{airfoil_suction_displacement_thickness_m_regression.png}
\caption{Observed ${y}$ and fitted $\hat{y}$ vs.\ \texttt{suction\_displacement\_thickness\_m} for Airfoil Self-Noise.}
\end{figure}


\subsection{Discussion}
Frequency and suction-side displacement thickness both show moderate negative association with sound pressure level: higher-frequency acoustic energy dissipates faster, and thicker boundary layers cushion surface interactions. The two univariate models explain only 15.3\% and 9.8\% of the variance respectively, reflecting that self-noise depends jointly on vortex shedding, boundary-layer separation, and attack angle -- physics that a single-variable linear model cannot capture, motivating multiple regression.
